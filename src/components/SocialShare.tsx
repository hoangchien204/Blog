...
const SocialShare: React.FC<Props> = ({ shareUrl, shareTitle }) => {
  ...
  const copyLink = async () => {
    try {
      ...
      setShowToast(true);
      setTimeout(() => setShowToast(false), 3000); // 3 giây sau biến mất
    } catch (err) {
    ...
    <style>
      {
        `
          ...
          0% {
            opacity: 0;
            transform: translate(-50%, -50px); /* ẩn trên trên */
          }
          20% {
            opacity: 1;
            transform: translate(-50%, 0); /* xuống vị trí chính giữa ngang, sát trên */
          }
          80% {
            opacity: 1;
            transform: translate(-50%, 0); /* đứng yên */
          }
          100% {
            opacity: 0;
            transform: translate(calc(-50% + 300px), 0); /* chạy sang phải rồi mờ dần */
          }
        }
